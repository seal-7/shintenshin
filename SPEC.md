...
- Append one synthetic line, schema-copied from a real `type:"user"` line: `{parentUuid: <uuid of last line having a uuid>, isSidechain:false, type:"user", message:{role:"user", content:"Shintenshin transfer complete. This conversation was imported from another machine or session — the filesystem, git state, and any files referenced above may not exist or may differ here. Re-verify file state before editing anything, then greet me, confirm the transfer succeeded, and summarize where we left off."}, uuid: <new random uuid>, timestamp: <now ISO>, sessionId: <new id>, cwd: <receiver cwd>, userType:"external", version: <copied from a prior line if present>, gitBranch:""}`
  - **Note**: this is plain visible text, not wrapped in `<system-reminder>` tags. Real system-reminders are only rendered invisibly by the CLI when nested inside a `tool_result` content block paired with a real preceding `tool_use` — faking that pairing risks a dangling `tool_use_id` reference breaking the transcript loader. A plain, clearly-worded visible message is the safe/correct approach.
- Write to `~/.claude/projects/<slug-of-receiver-cwd>/<new-id>.jsonl` (mkdir -p). NEVER overwrite an existing file.
- Print: success, the new session id, and next step. Recommended path: `/resume` (a real built-in interactive Claude Code slash command) run in the *same* terminal, picking the just-arrived session (most recent in the picker) — this replaces the active conversation in place without leaving the terminal. Fallback: `claude --resume <new-id>` from a new terminal. Note `/resume` is a manual interactive keystroke (arrow-key picker); it cannot be triggered programmatically by a script or tool call, so full auto-resume-in-place is not possible — this is the closest available UX.
